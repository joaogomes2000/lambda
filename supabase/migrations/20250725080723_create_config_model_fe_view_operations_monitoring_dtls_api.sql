CREATE OR REPLACE VIEW config_model.fe_view_operations_monitoring_dtls_api AS
    SELECT correlation_id AS execution_id,
           domain_name AS business_object,
           url AS operation_object,
           ...
           END AS status,
           CASE
               WHEN status::text IN ('E', 'W', 'P') THEN ((correlation_id::text || '_'::text) || id) || '.csv'::text
               ELSE NULL::text
           END AS filename
